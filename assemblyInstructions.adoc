...
==== Prepare parts for Rebar Stake part(s)
...
. Using the *5mm Allen Wrench*, screw in the *M6 x 20mm Screw* enough to hold the part(s) and *M6 Thin Nut* in place. Leave room for the *Rebar Stake* to be inserted later.
...
. Reproduce the Plate Jig SVG file that is located in the 'source' folder of the project documentation. The jig is a 300mm x 300mm square with 5mm radius holes centered at (15,15) (15,285) (285,15) and (285, 285).
...
== Assemble Universal Controller 120v Power Cord
...
==== Wire the Universal Controller 120v Power Cord
...
. Take the *Three Prong Electrical Cord* and strip back 1-2cm of sheathing using *Wire Strippers/Cutters*.
...
== Assemble Fast Heated Bed 120v Power Wire
...
==== Assemble the Fast Heated Bed 120v Power Wire
...
. Using the *Wire Strippers/Cutters*, cut one 200mm long piece of *Red 18AWG Wire*.
. Using the *Wire Strippers/Cutters*, cut one 750mm long piece of *Red 18AWG Wire* and one 750mm long piece of *Black 18AWG Wire*.
. Using the *Wire Strippers/Cutters*, strip 2cm of insulation off of each end of the *Black 18AWG Wire* and *Red 18AWG Wire* from the above steps.
. Using the *Dupont Crimper*, crimp one *Fork Terminal* onto one end of each of the *Black 18AWG Wire* and *Red 18AWG Wire* from the above steps.
. Using the *Dupont Crimper*, crimp one *Fork Terminal* onto the bare end of the 200mm *Red 18AWG Wire*. Set this wire aside as it is completed.
. Using the *Wire Strippers/Cutters*, cut off the plastic shield end of two *Ferrule* part(s) so that only the metal *Ferrule* tubes remain.
. Using the *Ferrule Crimper*, crimp the *Ferrule* tubes to the bare ends of the remaining *Black 18AWG Wire* and *Red 18AWG Wire*.
. The *Fast Heated Bed 120v Power Wire* is complete.
...
== Assemble 24v Power Supply 120v Power Wire
...
==== Assemble the 24v Power Supply 120v Power Wire
...
. Using the *Wire Strippers/Cutters*, strip 2cm of insulation off of each end of the *Red 22AWG Wire* and *Black 22AWG Wire* part(s) from the above steps.
. Using the *Dupont Crimper*, crimp one *Fork Terminal* onto one end of each of the above *Red 22AWG Wire* and *Black 22AWG Wire* part(s).
. Using the *Ferrule Crimper*, crimp one *Ferrule* onto one end of each of the above *Red 22AWG Wire* and *Black 22AWG Wire* part(s).
. The *24v Power Supply 120v Power Wire* is complete.
...
== Assemble Universal Controller Electrical Plug
...
==== Wire the Universal Controller Electrical Plug
...
== Assemble Universal Controller Emergency Switch
...
==== Wire the Universal Controller Emergency Switch
...
. TODO: Add inline fuse holder, fuse, and 18awg wire components
. Solder the inline fuse onto the extended power wire (still not ground).
. Solder the inline fuse to one of the tongs of the rocker switch.
. Solder a 200mm extension wire onto the other tong of the emergency-switch switch.
. TODO
...
== Assemble RAMPS 1.4 Board Assembly Heated Bed Signal Wire
...
==== Assemble the RAMPS 1.4 Board Assembly Heated Bed Signal Wire
...
. The *RAMPS 1.4 Board Assembly Heated Bed Signal Wire* is complete.
...
== Assemble RAMPS 1.4 Board Assembly Power Wire
...
==== Assemble the RAMPS 1.4 Board Assembly Power Wire
...
. Using the *Wire Strippers/Cutters*, cut one 50mm long piece of *Red 22AWG Wire* and one 50mm long piece of *Black 22AWG Wire*.
...
. The *RAMPS 1.4 Board Assembly Power Wire* is complete.
...
== Assemble RAMPS 1.4 Board Assembly
...
==== Remove the D1 diode from the RAMPS 1.4 Board
...
. Orient the *RAMPS 1.4 Board* with the screw-in terminals facing left and the row of pins on the right. This orientation will be assumed for the remaining steps in this section. 
. Locate the D1 diode on the *RAMPS 1.4 Board*. It is located between the slots for the X and Y *A4988 Stepper Motor Driver* part(s), below the capacitors.
. Using the *Flush Cutter Wire Snips*, cut the D1 diode off of the *RAMPS 1.4 Board*.
...
. Remove the sticker cover from a *Stepper Motor Driver Heatsink*.
. Stick the *Stepper Motor Driver Heatsink* on the black chip in the center of a *A4988 Stepper Motor Driver*.
. Repeat the above steps for each *A4988 Stepper Motor Driver*.
. Orient the *RAMPS 1.4 Board* with the screw-in terminals facing left and the row of pins on the right. This orientation will be assumed for the remaining steps in this section.
. Orient a *A4988 Stepper Motor Driver* with the white potentiometer on the *RIGHT* side.
. Insert the *A4988 Stepper Motor Driver* into the E0 slot of the *RAMPS 1.4 Board*. Orientation is VERY IMPORTANT as the *A4988 Stepper Motor Driver* can be inserted backwards which will DESTROY the electronics when powered on.
. Repeat the above steps for the X, Y, Z, and E1 slots in the *RAMPS 1.4 Board*. The E1 slot is used to drive the Y2 axis.
. Align the *RAMPS 1.4 Board* and *Arduino Mega 2560* such that the *Arduino Mega 2560* is directly above the *RAMPS 1.4 Board* and gently seat the pins into their sockets. There is only one possible orientation that allows both components to sit directly above one another.
. Using even pressure, push down on each corner of the *RAMPS 1.4 Board* until its pins are fully seated into the *Arduino Mega 2560*.
. Position the *RAMPS 1.4 Smart Adapter* over the right side of the *RAMPS 1.4 Board* so that both sets of sockets on the *RAMPS 1.4 Smart Adapter* align with the pins on the *RAMPS 1.4 Board*. There is only one orientation that will fit all pins properly.
. Using even pressure on both ends of the *RAMPS 1.4 Smart Adapter*, push the part gently into place over the *RAMPS 1.4 Board* until both components are seated against one another.
. The *RAMPS 1.4 Board Assembly* is complete.
...
==== Flash firmware on the RAMPS Board Assembly
...
. Plug the *RAMPS Board Assembly* into the *Personal Computer* with a *USB A to USB B Cable*.
. Download and install *Arduino IDE* from the https://www.arduino.cc/en/software[Arduino website]. (https://www.arduino.cc/en/software)
. Open the *Arduino IDE* software.
. On the top menu, click 'File' > 'Open...' and navigate to the project documentation folder. 
. To load the *Marlin Firmware* into the *Arduino IDE*, navigate to 'D3D Pro Marlin/Marlin' and select the 'Marlin.ino' file.
. Select *Arduino Mega 2560* from the boards menu at the top of the *Arduino IDE*.
...
. Download and install the *Raspberry Pi Imager* software from the https://www.raspberrypi.com/software/[Raspberry Pi Website] (https://www.raspberrypi.com/software/) onto the Personal Computer.
. Insert the *16GB MicroSD Card* into the *Personal Computer*. If necessary, use a MicroSD to SD Card Adapter or other compatible adapter.
. Open the *Raspberry Pi Imager* software and click 'Choose Device'. Select the appropriate Raspberry Pi Computer from the manu.
. Next, click 'Choose OS' > 'Other specific-purpose OS' > '3D Printing' > 'OctoPi' > 'OctoPi (Stable)'.
. Click 'Choose Storage' next, and select the *16GB MicroSD Card* from the list.
...
. The last step explains that ALL DATA on the *16GB MicroSD Card* will be erased by this operation. 
. Click 'Yes' to confirm the operation, erase all data on the *16GB MicroSD Card*, and install the *Octoprint Raspberry Pi Software Image* onto the *16GB MicroSD Card*.
. If prompted by the *Personal Computer*, authorize the operation (usually by entering a password).
. The *16GB MicroSD Card* is ready for use in the *Raspberry Pi Computer*.
...
. The below image shows the layout of the components within the *Universal Controller Case*.
+
image:IMG_3718.jpeg[,600,600,pdfwidth=60%] 
. Zip tie the *RAMPS Board Assembly* in the four bottom holes using *Small Zip Tie*.
. Zip tie the *24v Power Supply Module* in the four bottom holes using *Small Zip Tie*.
. Zip tie the *Raspberry Pi Computer* around the center using *Large Zip Tie*.
. Zip tie the *Solid State Relay (SSR)* through the mounting holes (if available) or around the center.
. Zip tie the *GFCI* from the back side of the case through the top and bottom mounting holes using *Small Zip Tie*.
...
. Connect the ferrule ends of the RAMPS Board Assembly to RAMPS Board Assembly 24v Power Custom Wires to the bottom left terminals, paying close attention to the polarity.
. Plug the 2-pin-dupont-connector end of the RAMPS Board Assembly to RAMPS Board Assembly 24v Power Custom Wires into the bottom left ports of the Aux 1 pins on the RAMPS board, with positive on the left and negative on the right.
...
. Follow the *Circular Saw* manufacturer's instructions to safely attach a *Metal Cutting Wheel* part to the *Circular Saw*.
. Secure the *Aluminum Stock* part in the *Vise* with most of the *Aluminum Stock* hanging out of the side of the *Vise*. 
. Use a *Measuring Tape* and a *Permanent Marker* to mark a 50mm cut from the end of the *Aluminum Stock* part. Make sure to leave enough room for the width of the *Metal Cutting Wheel* on the *Circular Saw*.
. While taking all appropriate precautions, use the *Circular Saw* to make the cut from the previous step.
. After the part cools from the cut, remove the remaining *Aluminum Stock* from the *Vise* and set it aside.
. If necessary, use the *Metal File* to remove burrs from the edges of the 50mm *Aluminum Stock* part.
...
. Bring the 50mm *Aluminum Stock* part from the previous step to the location of the *Drill Press*.
. While following the manufacturer's instructions for the *Drill Press*, securely install the *5mm Drill Bit* into the *Drill Press*.
. Align the *Heatsink Block Jig* over the 50mm *Aluminum Stock* part.
. Use the mounting tools included with the drill press (or a suitable alternative) to secure the 50mm *Aluminum Stock* part in place for the first hole to be drilled.
. While taking every necessary precaution, turn on the drill press and use it to drill the hole through the 50mm *Aluminum Stock* part.
. Repeat the above steps for all three holes that need to be drilled into the Universal Gearless Extruder Heatsink Block.
...
. Following the manufacturer's instructions, install the *M6 tap* into the *Tap Handle*.
. Secure the *Universal Gearless Extruder Heatsink Block* in the jaws of the *Vise* with the widest side of the *Universal Gearless Extruder Heatsink Block* facing up. The smallest hole should be accessible from the top.
. Using the *Tap Handle* and the *M6 tap*, begin to tap the hole.
. For best results, unscrew the *M6 tap* periodically while tapping the *Universal Gearless Extruder Heatsink Block*. This allows the metal shavings to fall out of the way.
. Once the hole is fully tapped, use the *Metal File* to remove any excess metal from the *Universal Gearless Extruder Heatsink Block*.
. The Universal Gearless Extruder Heatsink Block is now complete.
...
. Choose a side of the *Universal Frame* where the mounting holes extend past the edge of the frame.
. Position the *Universal Y1 Axis* so that the clips on the carriage are pointing up and the motor side is on the left.
. Align the holes in the motor side with the holes in the *Universal Frame*, and insert a *M6 x 25mm Screw* into both holes from behind.
. Insert two *M6 Nut* part(s) into the front mounting holes on the motor side of the *Universal Y1 Axis*.
. Tighten the *M6 x 25mm Screw* part by hand so the *Universal Y1 Axis* stays in palce.
. Loosen the set screws on the idler side of the *Universal Y1 Axis* using a *3mm Allen Wrench* and slide the idler side along the axis until the mounting holes align with the mounting holes on the *Universal Frame*.
. Tighten the set screws on the idler side using a *3mm Allen Wrench* to secure its alignment with the *Universal Frame*
. Insert a *M6 x 25mm Screw* through the mounting holes on the *Universal Frame* and into the idler side of the *Universal Y1 Axis*.
. Insert two *M6 Nut* part(s) into the mounting holes on the front of the idler side.
. Tighten the *M6 x 25mm Screw* part by hand.
. Loosen the *M6 x 25mm Screw* part slightly so that the *Universal Y1 Axis* can move and push down on each end of the *Universal Y1 Axis* so that it is seated evenly across it's length. Skipping this step can lead to misalignment.
. Using a *5mm Allen Wrench*, tighten down the *M6 x 25mm Screw* part on each side.
. The *Universal Y1 Axis* is now mounted.
...
==== Mount the Universal Z Axis module(s)
...
==== Mount the Universal Controller
...
==== Install the RAMPS 1.4 Endstop part(s)
...
==== Wire the Axes and RAMPS 1.4 Endstop part(s)
...
==== Install and Wire the Fast Heated Bed
...
==== Install and Wire the Universal Gearless Extruder
...
==== Test the wireless printer management software (Octoprint Raspberry Pi Software Image)
...
==== Test the Heaters, Motion System, and Auto-Home
...
==== Run automatic calibration of Fast Heated Bed
...
==== Set up slicer software and prepare a test print
...
==== Upload and print the test print (SD Card Method)
...
==== Upload and print the test print (Wireless Printing Method)
